...
def _R(name: str) -> sp.Expr:
    """レート式 alpha_m__hh(V) 等。レート関数は数値安定な exp 実装を壊さないため未定義
...
def _rate_types(model: str, gates: str) -> dict[str, list[LibraryEntry]]:
    ...
    レート名 alpha_m__hh は実装関数名かつ数式シンボル名: sympy の上付き記法に
    載せることで、モデルを区別しつつ α のギリシャ文字化を保つ (→ α^hh_m)。"""
    pairs = [(f"alpha_{g}__{model}", f"beta_{g}__{model}") for g in gates]
    return {
