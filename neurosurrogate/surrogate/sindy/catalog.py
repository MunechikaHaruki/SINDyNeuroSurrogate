...
from ..ansatz.roles import G, U, V  # 役割記号: 電位 / ゲート(=隠れ変数) / 外部電流
...
def _R(name: str) -> sp.Expr:
    """レート式 alpha_m_hh(V) 等。レート関数は数値安定な exp 実装を壊さないため未定義
...
def _rate_types(model: str, gates: str) -> dict[str, list[LibraryEntry]]:
    """1 モデルのレート項 type 一式 (モデル名 + ゲート文字列から派生)。type 同士は
    互いに素 (同じ式を2つの type が持たない) で、合成は yaml 側の責務。
    例: forward + backward = 全レート、forward + relaxation_decay = 緩和形。
    レート名 alpha_m_hh は実装関数名かつ数式シンボル名: モデル名を下付きへ回す
    ことで、区別しつつ α のギリシャ文字化を保つ (→ α_{m hh})。"""
    pairs = [(f"alpha_{g}_{model}", f"beta_{g}_{model}") for g in gates]
    return {
        # 順方向レート α(V)。緩和形 dg/dt = α - (α+β)·g の駆動項でもある
        f"{model}_gate_forward": [_entry(_R(a)) for a, _ in pairs],
        # 逆方向レート β(V)
        f"{model}_gate_backward": [_entry(_R(b)) for _, b in pairs],
        # α(V)·g
        f"{model}_gate_forward_product": [_entry(_R(a) * G) for a, _ in pairs],
        # β(V)·g。forward_product と組で dg/dt = α(1-g) - βg のゲート依存側を張る
        f"{model}_gate_backward_product": [_entry(_R(b) * G) for _, b in pairs],
        # 緩和形の減衰項。α·g と β·g を束ね、係数 1 個で真の構造に一致させる
        f"{model}_relaxation_decay": [_entry((_R(a) + _R(b)) * G) for a, b in pairs],
    }


# ゲート文字列がモデル毎の唯一の入力。HH=3ゲート、Traub=V 依存 8 ゲート
# (Q は XI 依存のため対象外)。
LIB_ENTRIES: dict[str, list[LibraryEntry]] = {
    **_rate_types("hh", "mhn"),
    **_rate_types("traub", "msncahrb"),
