...
from ..preprocessor.fit_artifacts import fit_artifacts
...
class UDEAnsatz(HybridAnsatz[UDEClosure]):
    ...
    def fit_closure(
        ...
        training_gates = cls.training_gates(spec, training_data)
        # hyperparams は closure ブロックを展開して渡す (この関数は閉包項の同定入口 =
        # SINDy の `from_sindy` と同じ立場) = **既定値は `_train_ude` の署名 1 箇所**
        # で、綴り違いは黙って既定へ落ちずに TypeError になる。
        return _train_ude(
            preprocessor,
            jnp.asarray(np.stack(training_gates), dtype=jnp.float32),
            jnp.asarray(
                np.stack(access.potentials(training_data, spec.train_comp_ids())),
                dtype=jnp.float32,
            ),
            np.concatenate(training_gates, axis=0),
    ...
    @classmethod
    def dlatent(
        cls, spec: SurrogateSpec, closure: UDEClosure
...
def _train_ude(
    ...
    *,
    ...
    """encoder/decoder/潜在 NN を同じロスで回す。

    **preprocessor を更新して**閉包項を返す。gate (n_comp, T, n_gate) / volt
    (n_comp, T) は窓を切る軌道、train_gate は再構成統計を取り直す素データ (comp 連結)。
    """
    ...
    params: dict[str, Any] = {
        ...
        "nn": _init_mlp(
            [n_latent + 1] + [hidden] * depth + [n_latent],
    ...
    def step(params, opt_state, key):
        ...
        (loss, parts), grads = jax.value_and_grad(loss_fn, has_aux=True)(params, gb, vb)
    ...
    preprocessor.gate_inits, preprocessor.reconstruction = fit_artifacts(
        preprocessor, train_gate
    )
