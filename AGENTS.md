...
| 探しているもの | 領域 |
| --- | --- |
| 修論の計画、進捗、修了要件 | 2025-MunechikaHaruki-master-thesis |
| 本人の事実、進路の判断基準 | context-router 直下の `profile.md`、`policy.md` |

概念の説明や教材は、この領域からは参照しない。**教材と研究コードは互いを見ないまま独立に保ち、
同じ内容の二重実装を許す。** 教材が要るときは context-router のルーティングから辿る。
