...
**scripts/**: `main.py`(Hydra エントリ) / `mlflow_io.py`(MLflow I/O。`log_surrogate_model` は `surrogate.save` + `meta.yaml`(surrogate_type/dataset/train_comp_id) を artifact log、`load_surrogate_model` は `(NeuroSurrogateBase, SurrogateMeta)` tuple 返却) / `analysis/`(marimo 層。`access.py`=base_ui read 規約集約 (leaf、`target_of`/`train_of`/`current_of`。ui→mode import と衝突せぬよう独立) / `ui.py`=widget builder / `view.py`=表示合成 (`view_result` が single/sweep 縦積み合成、Panel 組立) / `save/`=永続化サブpkg (`panel.py`=Panel 表示 accumulator+SaveEntry+保存機構 / `restore.py`=状態 snapshot/復元。`to_meta` UI値→meta.json (run 選択は run_id 列で round-trip 可)、`make_panel` 復元 widget、`load`) / `mode/{single,sweep}.py`=per-mode compute+draw (`calc_eval`/`calc_sweep` = 置換後シミュ。calc gate と run load は薄 wrapper 廃止し marimo.py セルに inline、run load は `mlflow_io.load_from_selector`)。全 UI builder は preset 引数で meta.json 復元値を初期値上書き。依存: view→ui/save/mode、access は leaf、循環無) / `marimo.py` / `conf/`(`config.yaml` に共通 `surrogate.fit.optimizer` + `init.n_components` 既定 (1) 集約 + `surrogate/*.yaml` プリセット (base=`hh_full`/`traub_full` は lib=full の素体、type/n_components/preprocessor は `hydra.sweeper.params` 既定の `--multirun` で直積に振る。lib 本質差の `_hh_informed`/`_hh_relaxation` のみ個別)。`n_components` は `init` 直下 (preprocessor spec でなく)、library_specs は type+latent序数 `latents` のみ (省略=全latent。V/u/定数は項が固定保有))
