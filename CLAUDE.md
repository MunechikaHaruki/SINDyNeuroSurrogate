...
**neurosurrogate/core/**: `network.py`(`Compartment`/`CompartmentType`/`NeuronGraph`/`DatasetConfig`/`CurrentConfig`。`chain(...)` ノード自動命名、`with_surrogate(new_type, accept)` で accept 真ノードをサロゲート置換) / `simulator.py`(`unified_simulator` + `calc_universal_deriv` が type 名 dispatch で `vmap` 一括計算、`generic_euler_solver` が `lax.scan`) / `coords.py`(xarray 座標) / `opcost.py`(`OpCost` 代数、`+`/`*` で演算コスト集計)
...
**neurosurrogate/surrogate/**: `base.py`(`NeuroSurrogateBase`、`_dataset`/`train_comp_id`/`_train_xr` 保持、`save`/`load` は `surrogate.joblib` 単一ファイル。置換妥当性は `SurrogateMeta.verdict(comp)` が学習ドメイン照合で3値判定 (`REPLACE` 型+params 一致 / `MISMATCH` 型一致・params 違い / `SKIP` 型違い)。`surrogate.apply(dataset)` が fail first: MISMATCH 有 or REPLACE 皆無で即 `ValueError`、REPLACE ノードのみ `surr_comp_type` (学習結果から構築するプロパティ) に置換。低レベル `net.with_surrogate(new_type, accept)` は accept 真ノードを `replace(n, type=new_type)` で置換 (name/params 保持し type だけ差替、`COMPARTMENT_TYPES` 共有参照で load 跨ぎ一致)。診断 `transform_gate` も verdict ガードで非 REPLACE を明示エラー化) / `sindy.py`(`SINDyNeuroSurrogate`) / `hybrid.py`(`HybridSINDyNeuroSurrogate`、AE preprocessor 利用可、n_components 3 が既定) / `bundle.py`(`PREPROCESSOR_CLS`={"pca","ae"} / `OPTIMIZER_CLS` / `SindyBundle`/`PreprocessorBundle`、`bundle.decode()` 統一) / `libraries.py`(`FeatureLibrary`/`SubLibrary`/`LibraryEntry`、`FIXED_LIB_ENTRIES`={hh_gate,hh_gate_product,hh_gate_forward,hh_gate_forward_product,hh_relaxation_driver,hh_relaxation_decay,volt,gate_poly_volt} + `VARIADIC_LIB_ENTRIES`={basis}、`FeatureLibrary.build(specs)`) / `autoencoder.py`(`AutoEncoderPreprocessor`、PCA 互換 JAX-AE) / `__init__.py` に `SURR_CLS` 辞書 (`sindy`/`hybrid`)
